...
# #101 A.1 -- the advertised list is DERIVED from the guard's own predicate. It previously carried
# two extra entries (memory/MEMORY.md, Plans/PLAN.md) that governance-guard.sh does not enforce, so
# the token documented a protection that did not exist. Deriving it makes that drift impossible.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=_common.sh
. "$SCRIPT_DIR/_common.sh" 2>/dev/null || true
if command -v gov_protected_patterns >/dev/null 2>&1; then
  PROTECTED_JSON=$(gov_protected_patterns | sed 's/^/    "/; s/$/",/' | sed '$ s/,$//')
  PROTECTED_SOURCE="_common.sh"
else
  # _common.sh unavailable: print NOTHING rather than a guessed list. The guard only reads
  # expires_at_epoch, so an empty informational list costs nothing -- a wrong one costs trust.
  PROTECTED_JSON=""
  PROTECTED_SOURCE="unavailable"
fi
