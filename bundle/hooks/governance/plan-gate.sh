...
# Read the payload once and resolve the project it is ABOUT. Sets GOV_PROJECT_ROOT.
# Without this the governed-project test below is relative to wherever this hook process
# happens to stand, which from inside `~/.claude/**` resolves to the USER-LEVEL CLAUDE.md.
gov_prime_payload

# Node-role gate (Framework v2): planning makes sense only on SOURCE.
...
# Skip cron-fired prompts (WA-CC bridge poll, scheduled tasks, etc.)
echo "$INPUT" | grep -q '^\[WA→CC Poll\]\|^\[WA->CC Poll\]\|^✓$' && exit 0
