...
if [ -f "$SCRIPT_DIR/_common.sh" ]; then
  ...
  gov_disabled && exit 0
  # Node-role gate (Framework v2): sync only runs on SOURCE.
  # DEPLOYMENT: UPDATE.bat is the single sync path — hook must not rewrite files
  # to match local version.json (which is a receipt, not SSOT, on DEPLOYMENT).
  if [ "$(gov_detect_role "$PROJECT_ROOT")" != "SOURCE" ]; then
    gov_log "sync-governance" "non-SOURCE role — skip"
    exit 0
  fi
else
...
if [ -f "$GOTCHAS_FILE" ]; then
  # Gotcha entries come in TWO shapes and BOTH must be counted:
  #   bare list item : "1. **...", "244. **..."
...
# Pattern: **<Project Name>** (v1.2.3) in the Project Overview line. The project name is
# CAPTURED, not hardcoded: this hook ships in a public bundle and must not carry one
# project's name. \1 replays whatever name the file already uses.
update_file \
  "$PROJECT_ROOT/CLAUDE.md" \
  "s|\*\*\([^*]\+\)\*\* (v[0-9]\+\.[0-9]\+\.[0-9]\+)|\*\*\1\*\* (v${CURRENT_VERSION})|g" \
...
if [ -f "$GOTCHAS_FILE" ] && [ "$GOTCHAS_COUNT" -gt 0 ]; then
  update_file \
    "$GOTCHAS_FILE" \
    "s|[0-9]\+ entries\.\([*_[:space:]]*\)\$|${GOTCHAS_COUNT} entries.\1|" \
