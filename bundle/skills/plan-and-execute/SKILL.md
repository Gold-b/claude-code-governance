...
9. **`docs/context/HANDOFF.md`** — lifecycle transition
10. **`docs/context/NEXT-SESSION-PROMPT.md`** — only if the plan's goal is still open AND the session is ending: rewritten from scratch per `~/.claude/docs/NEXT-SESSION-HANDOVER.md` (orchestrator Step 6.1). A plan that finished all its phases and verified them gets **no** continuation prompt — and any leftover file from a previous goal is deleted, not left in place.
